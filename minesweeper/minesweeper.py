    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Ignore the cell itself and known safe cells
                if (i, j) == cell or (i,j) in self.safes:
                    continue

                # Factor in known mines
                if (i, j) in self.mines:
                    count -= 1
                    continue
                
                # Ensure only cells that exist  are added
                if 0 <= i < self.height and 0 <= j < self.width:
                            undetermined_cells.add((i, j))

        # Add valid cells to knowledge base
        if undetermined_cells != set():
            self.knowledge.append(Sentence(undetermined_cells, count))

        # 4
        # Check knowledge base using new knowledge
        for sentence in self.knowledge:
            # Scan for and mark mines
            mines = sentence.known_mines()
            if mines:
                mines_copy = mines.copy()
                for check_cell in mines_copy:
                    self.mark_mine(check_cell)

            #Scan for and mark safe spots
            safes = sentence.known_safes()
            if safes:
                safes_copy = safes.copy()
                for check_cell in safes_copy:
                    self.mark_safe(check_cell)
        ...
        for sentence_a in self.knowledge:
            for sentence_b in self.knowledge:
                ...
                # A is superset of B
                elif sentence_a.cells > sentence_b.cells:
                    # print('A > B')
